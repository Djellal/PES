...
namespace Pes.Pages
{
    public partial class EditElementComponent : ComponentBase, IDisposable
    {
        ...
        {
            var dMdelGetElementByIdResult = await DMdel.GetElementById(Id);
            element = dMdelGetElementByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Pes.Models.DMdel.Element args)
        {
            try
            {
                var dMdelUpdateElementResult = await DMdel.UpdateElement(Id, element);
                DialogService.Close(element);
            }
            catch (System.Exception dMdelUpdateElementException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Element" });
            }
        }
