...
namespace Pes.Pages
{
    public partial class EditFacultesComponent : ComponentBase, IDisposable
    {
        ...
        {
            var dMdelGetFaculteByIdResult = await DMdel.GetFaculteById(Id);
            faculte = dMdelGetFaculteByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Pes.Models.DMdel.Faculte args)
        {
            try
            {
                var dMdelUpdateFaculteResult = await DMdel.UpdateFaculte(Id, faculte);
                DialogService.Close(faculte);
            }
            catch (System.Exception dMdelUpdateFaculteException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Faculte" });
            }
        }
