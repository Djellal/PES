...
namespace Pes.Pages
{
    public partial class AddCritereComponent : ComponentBase, IDisposable
    {
        ...
        {
            critere.Elementid = int.Parse($"{Elementid}");

            try
            {
                var dMdelCreateCritereResult = await DMdel.CreateCritere(critere);
                DialogService.Close(critere);
            }
            catch (System.Exception dMdelCreateCritereException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Critere!" });
            }
        }
