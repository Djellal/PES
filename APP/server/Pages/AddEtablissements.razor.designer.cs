...
namespace Pes.Pages
{
    public partial class AddEtablissementsComponent : ComponentBase, IDisposable
    {
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Pes.Models.DMdel.Etablissement args)
        {
            try
            {
                var dMdelCreateEtablissementResult = await DMdel.CreateEtablissement(etablissement);
                DialogService.Close(etablissement);
            }
            catch (System.Exception dMdelCreateEtablissementException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Etablissement!" });
            }
        }
